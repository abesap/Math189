...
def find_RMSE(W, X, y):
	...
	meansqerror = np.linalg.norm(difference,2)**2/num_elements
	"*** END YOUR CODE HERE ***"
	return np.sqrt(meansqerror)
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	for reg_value in reg_list:
		W_opt = linreg(X_train, y_train, reg=reg_value)
	...
	return reg_list[np.argmin(RMSE_list)]
...
def linreg_no_bias(X, y, reg=0.0):
	...
	dim = X.shape[0]
	ident = np.eye(dim)
	tempA = X.T @ (ident- np.ones(dim)/dim)
	W_opt= np.linalg.solve(tempA @X+reg * np.eye(tempA.shape[0]), tempA @y)
	b_opt = sum(y-X@W_opt)/dim
	"*** END YOUR CODE HERE ***"
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		""" I had to look at the solution key to understand what exaclty the 
		 W_grad and b_grad values should be set to. I also found that the code 
		 does not work without the reshaping but I am not quite sure why. I will
		 attend and ask during Joe's tutoring hours. """
		"*** YOUR CODE HERE ***"
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	ones_x_train = np.ones_like(X_train)
	ones_x_test = np.ones_like(X_test)
	ones_x_val = np.ones_like(X_val)
	X_train = np.hstack((ones_x_train, X_train))
	X_test = np.hstack((ones_x_test, X_test))
	X_val = np.hstack((ones_x_val, X_val))
	"*** END YOUR CODE HERE ***"
