...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		"*** YOUR CODE HERE ***"
		##Label w/ current clusters
		for x in range(m):
			difference = X[x,:]-clusters
			label[x] = (np.argsort(np.linalg.norm(difference, axis= 1))).item(0)
		for x in range(k):
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	CostvsNumclust, = plt.plot(range(1,21),cost_k_list,'black')
	opt_kplot, = plt.plot(opt_k,min(cost_k_list), 'bo')
	"*** END YOUR CODE HERE ***"
	...
	X_clusts = clusters[label.flatten()]
	data, = plt.plot(X[:,0], X[:,1], 'bo')
	clust, = plt.plot(X_clusts[:,0], X_clusts[:,1], 'rx')
	"*** END YOUR CODE HERE ***"

	# set up legend and save the plot to the current folder
	plt.legend((data, clust), \
