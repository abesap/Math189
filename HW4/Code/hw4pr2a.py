...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	sigma = sigmoid(X@W)
	tosum = np.multiply(y,np.log(sigma))+ np.multiply((1-y),np.log(1-sigma))
	nll = np.sum(tosum)+reg/2*np.linalg.norm(W)
	"*** END YOUR CODE HERE ***"
	return nll
...
def newton_step(X, y, W, reg=0.0):
	...
	### Had to look a solution code for the np.squeze term
	diag = np.diag(np.squeeze(np.multiply(sigma, (1-sigma))))
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		if np.isnan(nll):
			break
		nll_list.append(nll)
		W_step = newton_step(X,y,W,reg=reg)
		W-=W_step
		"*** END YOUR CODE HERE ***"
...
def get_description(X, y, W):
	...
	while index < m:
		...
		if (predicted == true):
			count_accurate += 1
		index +=1
	accuracy = count_accurate/(index-1)
	precision = true_pres/pred_pres
	recall = pred_recal/true_recal
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	for x in range(len(reg_list)):
		reg= reg_list[x]
		W_opt, obj = grad_descent(X_train, y_train, reg = reg, print_freq = 500)
